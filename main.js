...
const fftSize = parseInt(Math.pow(2, parseInt(urlParams.get('fftSize') ?? '12')));
const showWave = parseInt(urlParams.get('showWave') ?? '1');

if (!showWave) canvas.style = 'display: none';

console.log(fftSize, showWave);
...
startButton.addEventListener('click', async () => {
	...
	function analyze() {
		...
		if (pitchData) noteView.innerHTML = `${pitchData.note}<sup>${pitchData.octave}</sup>`;
...
/**
 * @param {number} freq
 * @returns {{note: string, octave: number, freq: number, accuracy: number} | null}
 */
function pitch(freq) {
...
startButton.addEventListener('click', () => {
	startButton.style.transform = 'scale(1.0)';
	startButton.style.color = '#1e1b1b';
